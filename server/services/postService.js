...
async function getByTag(tagId){
    try {
        ...
        const allPosts = await tag.getPosts({include: [db.user, db.tag]});
            /*Kontrollera allPosts */
            /*Om något blev fel returnera felmeddelande */
...
async function addComment(id, comment) {
    ...
    if(!id) {
        return createResponseError(422, "ID är obligatoriskt nörd.")
    }   
    try {
        comment.postId = id;
...
async function update(post, id){
    ...
    try {
        ...
        if(!existingPost) {
            return createResponseError(404, "Hittade inget inölägg att uppdataera")
        }
        ...
        return createResponeMessage(200, 'Inlägget uppdaterades')   
        } catch (error) {
            return createResponseError(error.status, error.message);
        }
}
...
async function destroy(id){
    ...
    try {
        ...
        return createResponeMessage(200, 'Inlägget raderades') 
    } catch (error) {
        return createResponseError(error.status, error.message);
    } 
}
...
module.exports= {
    getByAuthor, getByTag, addComment, getById, getAll, create, update, destroy
}